...
/*
命令行参数结构体
blk_or_inode 输入为块号还是inode号 0表示块号，1表示inode
*/
struct globalArgs_t {
	int blk_or_inode;          /* -i or -b*/
...
{
	char* dev_name;
	struct data_list data_head;
	struct data_list* data_curr;
...
struct blkid_cache{
	...
	char* dev_name;
	char* file_path;
	struct blkid_cache* next;
};

static struct inode_cache* g_ino_cache_head = NULL;
static struct blkid_cache* g_blk_cache_head = NULL;

struct inode_cache* find_inode_cache(struct inooffset_strde_cache* head, char* dev_name, blk64_t inode_num){
	struct inode_cache* cur = head;
	while(cur != NULL){
		if(cur->inode_num == inode_num && strcmp(cur->dev_name, dev_name) == 0){
			break;
		}
		cur = cur->next;
	}
...
void insert_inode_cache(struct inode_cache** head, char* dev_name, char* file_path, blk64_t inode_num){
	struct inode_cache* new_node_cache = malloc(sizeof(struct inode_cache)); 
	new_node_cache->dev_name = malloc(strlen(dev_name) + 1);
	memset(new_node_cache->dev_name, strlen(dev_name) + 1, 0);
	memcpy(new_node_cache->dev_name, dev_name ,strlen(dev_name));
	if(file_path){
		new_node_cache->file_path = malloc(strlen(file_path) + 1);
		memset(new_node_cache->file_path, strlen(file_path) + 1, 0);
		memcpy(new_node_cache->file_path, file_path ,strlen(file_path));
	}
	else {
		new_node_cache->file_path = NULL;
	}
	new_node_cache->inode_num = inode_num;
	new_node_cache->next = NULL;
	if(*head == NULL){
		*head = new_node_cache;
	}
	else{
		struct inode_cache* tmp = (*head)->next;
		(*head)->next = new_node_cache;
		new_node_cache->next = tmp;
	}
}


void free_inode_cache(struct inode_cache* head)
{
...
释放链表
*/
void free_dev_list(struct dev_list *head)
{

}
/*
 * 从设备链表中查找相关文件名的设备
 * @param node 开始查找的起始节点 [输入参数]
 * @param dev 设备名 [输入参数]
 * @return 如果查找成功，返回节点，失败则返回空
 */
struct dev_list* find(struct dev_list* node, char *dev)
{
	if(!node) {
		return NULL;
	}
	if(!strcmp(node -> dev_name, dev)) {
		return node;
	}
	else {
		return find(node -> next, dev);
	}
}
struct input_file{
	char *num;
...
static int check_input(struct input_file input)
{
	int tmp_flg = (input.pro_name && input.offset && globalArgs.blk_or_inode) || (!input.pro_name && !input.offset && !globalArgs.blk_or_inode);
	if(!(input.num && input.dev && input.timestamp && input.rw && input.size && tmp_flg)) {
		return -1;
	}
	return 0;
}
 ...
 * @param size    大小
 */
void insert(struct dev_list* head, struct input_file input)
{	
	blk64_t blk_num = 0;
	blk64_t offset = 0;
	ext2_ino_t ino_num = 0;
	int err;
	if(globalArgs.blk_or_inode) {
		ino_num = strtoul(input.num, NULL, 0);
		if(!ino_num) {
			com_err(__func__, 0, "can not convert %s to inode number!\n", input.num);
			exit(1);
		}
		err = strtoint(__func__, input.offset, &offset);
		if (err) {
			com_err(__func__, 0, "can not convert %s to int!\n", input.offset);
			exit(-1);
		}
	} 
	else {
		err = strtoint(__func__, input.num, &blk_num);
		//blk_num cannot be zero.
		if (err || !blk_num) { 
			com_err(__func__, 0, "can not convert %s to block number!\n", input.num);
			exit(-1);
		}
	}

	struct dev_list* node = head;
	
	if(!(head -> dev_name)) {
		head -> dev_name = malloc(strlen(input.dev) + 1);
		memcpy(head -> dev_name, input.dev ,strlen(input.dev) + 1);
		head -> next = NULL;
		head -> data_head.blk_num = blk_num;
		head -> data_head.ino_num = ino_num;
		head -> data_head.timestamp = atof(input.timestamp);
		head -> data_head.opreation = input.rw[0];
		if(globalArgs.blk_or_inode) {
			head -> data_head.offset = offset;
			head -> data_head.pro_name = malloc(strlen(input.pro_name) + 1);
			memcpy(head -> data_head.pro_name, input.pro_name ,strlen(input.pro_name) + 1);
		}
		else{
			head -> data_head.offset = 0;
			head -> data_head.pro_name = NULL;
		}
		head -> data_head.next = NULL;
		head -> data_head.path = NULL;
		head -> data_head.size = atoi(input.size);
		head -> data_curr = &(head -> data_head);
		return;
	}
	node = find(node, input.dev);
	if(!node) {
		node = head;
		while(node -> next) {
			node = node -> next;
		}
		node -> next = malloc(sizeof(struct dev_list));
		node = node -> next;
		node -> dev_name = malloc(strlen(input.dev) + 1);
		memcpy(node -> dev_name, input.dev ,strlen(input.dev) + 1);
		node -> data_curr = &(node -> data_head);
	}
	else {
		node -> data_curr -> next = malloc(sizeof(struct data_list));
		node -> data_curr = node -> data_curr -> next;
	}
	node -> data_curr -> blk_num = blk_num;
	node -> data_curr -> ino_num = ino_num;
	node -> data_curr ->timestamp = atof(input.timestamp);
	node -> data_curr ->size = atoi(input.size);
	node -> data_curr ->opreation = input.rw[0];

	if(globalArgs.blk_or_inode) {
		node -> data_curr ->offset = offset;
		node -> data_curr ->pro_name = malloc(strlen(input.pro_name) + 1);
		memcpy(node -> data_curr ->pro_name,input.pro_name ,strlen(input.pro_name) + 1);
	}
	else{
		node -> data_curr ->offset = 0;
		node -> data_curr ->pro_name = NULL;
	}
	node -> data_curr ->path = NULL;
	node -> data_curr ->next = NULL;	
}
 ...
 * @param head        链表头节点    
 */
void save_csv(char *filepath, struct dev_list *head)
{
	FILE *fp;    
	...
	struct dev_list *node = head;
	if(!node || !(node -> dev_name))
		return;

	while(node) {
		struct data_list *data_node = &(node -> data_head);
		char *out_buf = malloc(MAX_LINE);
		memset(out_buf, 0, MAX_LINE);
		while(data_node) {
			sprintf(out_buf, "%s%lf,",out_buf, data_node->timestamp);
			if(data_node->path)
				sprintf(out_buf, "%s%s,", out_buf,data_node->path);
			else
				sprintf(out_buf, "%snull,", out_buf);
			if(data_node->pro_name)
				sprintf(out_buf, "%s%s,", out_buf,data_node->pro_name);
			else
				sprintf(out_buf, "%snull,", out_buf);
			sprintf(out_buf, "%s%c,",out_buf,  data_node->opreation);
			sprintf(out_buf, "%s%d", out_buf, data_node->size);
			if(globalArgs.blk_or_inode)
				sprintf(out_buf, "%s,%d", out_buf, data_node->offset);
			//fprintf(fp, "%s\n", out_buf);
			sprintf(out_buf, "%s\n", out_buf);
			char *p = malloc(strlen(out_buf) + MAX_LINE);
			memcpy(p, out_buf, strlen(out_buf) + 1);
			free(out_buf);
			out_buf = p;
			int i = strlen(out_buf);
 ...
 * @param head   链表头节点    
 */
void print_dev_list(struct dev_list *head)
{
	struct dev_list *node = head;
 ...
 * @return  文件路径字符串
 */
char* convert(blk64_t blk, char *dev)
{
	int		    retval = 0;
	int		    open_flags = EXT2_FLAG_SOFTSUPP_FEATURES | EXT2_FLAG_64BITS;
	blk64_t		superblock = 0;
	...
	char		*data_filename = 0;
	char		*undo_file = NULL;
	char        *debug_prog_name = "test";
	
	open_filesystem(dev, open_flags,
		superblock, blocksize, catastrophic,
		data_filename);
	ext2_ino_t ino = do_icheck_v(blk);
	char *path = do_ncheck_v(ino);
	close_filesystem();
	return path;
}
 ...
 * @param head  链表头节点
 */
void find_filename(struct dev_list * head)
{
	int		    retval = 0;
	int		    open_flags = EXT2_FLAG_SOFTSUPP_FEATURES | EXT2_FLAG_64BITS;
	blk64_t		superblock = 0;
	...
	if(!node || !node -> dev_name)
		return;
	while(node) {
		open_filesystem( node->dev_name, open_flags,	
			superblock, blocksize, catastrophic,
			data_filename);
		struct data_list *data_node = &(node -> data_head);
		while(data_node) {
			if(globalArgs.blk_or_inode)
			{
				struct inode_cache* cache_node = find_inode_cache(g_ino_cache_head, node->dev_name, data_node->ino_num);
				if (cache_node != NULL){
					data_node->path = cache_node->file_path;
				}
				else{
					data_node->path = do_ncheck_v(data_node->ino_num);
					insert_inode_cache(&g_ino_cache_head, node->dev_name, data_node->path, data_node->ino_num);
				}
			}
			else{
				ext2_ino_t ino = do_icheck_v(data_node->blk_num);
				data_node->path = do_ncheck_v(ino);
			}
			data_node = data_node -> next;
		}
		node = node -> next;
...
/*
 * 数据文件读取函数
 * 本函数直接读取全局变量globalArgs
 * @return  包含块号数组
 */
 struct dev_list readconf()
{
	char *filepath = globalArgs.in_file_name;
	...
	while (!feof(fd))
    {   
		fgets(linebuf,MAX_LINE,fd);
		printf("%s\n", linebuf);
		char *delim = ",";
		struct input_file input_str;
		if(globalArgs.blk_or_inode){ //inode方法: 时间戳:inode:pro_name:挂载点:读写:偏移:大小
			input_str.timestamp = strtok(linebuf, delim);
			input_str.num = strtok(NULL, delim);
			input_str.pro_name = strtok(NULL, delim);
			input_str.dev = strtok(NULL, delim);
			input_str.rw = strtok(NULL, delim);
			input_str.offset = strtok(NULL, delim);
			input_str.size = strtok(NULL, delim);
		}
		else { //block方法 blk:挂载点:时间戳:读写:大小
			input_str.num = strtok(linebuf, delim);
			input_str.dev = strtok(NULL, delim);
			input_str.timestamp = strtok(NULL, delim);
			input_str.rw = strtok(NULL, delim);
			input_str.size = strtok(NULL, delim);
			input_str.pro_name = strtok(NULL, delim);;
			input_str.offset = strtok(NULL, delim);;
...
{
	const char * usage = "Usage: %s ib <input file> <output file>";
	if(argc != 4){
		com_err(argv[0], 0, usage, argv[0]);
		return 0;
	}

	/* Initialize globalArgs before we get to work. */
	globalArgs.blk_or_inode = 0;     /* false */
    globalArgs.in_file_name = NULL;
	globalArgs.out_file_name = NULL;
	
	int c = 0;
	const char *opt_string = "i:b:";
	
	while ((c = getopt (argc, argv, opt_string)) != EOF) {
		
		switch (c) {
			case 'i':
				globalArgs.blk_or_inode = 1;
				globalArgs.in_file_name =  optarg;
				break;
			case 'b':
				globalArgs.blk_or_inode = 0;
				globalArgs.in_file_name =  optarg;
				break;
			default:
				com_err(argv[0], 0, usage, argv[0]);
	...
	globalArgs.out_file_name = argv[optind];
	struct dev_list head = readconf();
	find_filename(&head);
	save_csv(globalArgs.out_file_name,&head);
	return 0;
}
