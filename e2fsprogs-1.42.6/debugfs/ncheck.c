		       ...
		       void	*private)
{
	...
	for (i=0; i < iw->num_inodes; i++) {
		...
		"while calling ext2fs_get_pathname for inode #%u", iw->dir);
			...
			if (iw->parent){
				size_t buffsize = (dirent->name_len & 0xFF) + strlen(iw->parent) + 2;
				path = (char *)malloc(buffsize);
				printf("%u\t%s/%.*s", iw->iarray[i],
				       iw->parent,
					   (dirent->name_len & 0xFF), dirent->name);
				path = (char *)malloc(buffsize);
				snprintf(path, buffsize,"%s/%.*s",
						iw->parent,
						(dirent->name_len & 0xFF),
						dirent->name);
			}
			else{
				size_t buffsize = (dirent->name_len & 0xFF) + strlen(iw->dir) + 2;
				path = (char *)malloc(buffsize);
...
char *do_ncheck_v(ext2_ino_t ino)
{
	path = NULL;
	struct inode_walk_struct iw;
	int			c, i;
